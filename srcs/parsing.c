...
void	ft_file_read(t_data *data, char *file_name)
{
	...
	{
		skip_lines(data);
		if (!(ft_strncmp("NO", data->line, 2)) && data->text_d.textures[1] == NULL)
			data->text_d.textures[1] = get_textures(data, skip_spaces(data, 2));
		else if (!(ft_strncmp("SO", data->line, 2)) && data->text_d.textures[3] == NULL)
			data->text_d.textures[3] = get_textures(data, skip_spaces(data, 2));
		else if (!(ft_strncmp("WE", data->line, 2)) && data->text_d.textures[2] == NULL)
			data->text_d.textures[2] = get_textures(data, skip_spaces(data, 2));
		else if (!(ft_strncmp("EA", data->line, 2)) && data->text_d.textures[0] == NULL)
			data->text_d.textures[0] = get_textures(data, skip_spaces(data, 2));
		else if (!(ft_strncmp("F", data->line, 1)) && data->text_d.floor_c == 0)
			data->text_d.floor_c = get_colors(data, 'F');
		else if (!(ft_strncmp("C", data->line, 1)) && data->text_d.sky_c == 0)
